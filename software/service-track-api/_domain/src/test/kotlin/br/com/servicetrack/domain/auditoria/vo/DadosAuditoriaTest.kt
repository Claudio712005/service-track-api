...
class DadosAuditoriaTest {
    ...
    @Test
    fun `evento deve retornar DadosAuditoria com alteracoes nulas`() {
        val dados = DadosAuditoria.evento()
        assertNull(dados.alteracoes)
    }

    @Test
    fun `evento deve retornar true em temAlteracoes quando alteracoes é nulo`() {
        val dados = DadosAuditoria.evento()
        assertTrue(dados.temAlteracoes())
    }

    @Test
    fun `atualizacaoSemAntes deve gerar campos com valorAntes nulo e valorDepois preenchido`() {
        val snapshot = Snapshot("1", "Produto", true, 10)
        val dados = DadosAuditoria.atualizacaoSemAntes(snapshot)
    ...
    @Test
    fun `remocaoSemEstado deve retornar campo estado com valorAntes ATIVO e valorDepois REMOVIDO`() {
